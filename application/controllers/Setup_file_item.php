...
class Setup_file_item extends Root_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message='';
        $this->permissions=User_helper::get_permission('Setup_file_item');
        $this->controller_url='setup_file_item';
    }
    ...
    private function system_add()
    {
        ...
        {
            $data['title']='Create New '.$this->lang->line('LABEL_FILE_ITEM');
            $data['item']=array
            (
                'id'=>0,
                'name'=>'',
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2']) && ($this->permissions['action2']==1))
        {
            ...
            $data['title']='Edit '.$this->lang->line('LABEL_FILE_ITEM').' ('.$data['item']['name'].')';
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update($this->config->item('table_fms_setup_file_item'),$data,array('id='.$id));
            }
            else
            {
                $data['user_created']=$user->user_id;
                $data['date_created']=time();
                Query_helper::add($this->config->item('table_fms_setup_file_item'),$data);
            }
